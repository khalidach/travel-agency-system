...
export default function BookingPage() {
  ...
  const handleSaveBooking = async (
    ...
    try {
      ...
      if (editingBooking) {
        const updatedBooking = await api.updateBooking(
          editingBooking.id,
          bookingToSave
        );
  ...
  const handleSavePayment = async (payment: Omit<Payment, "_id" | "id">) => {
    if (selectedBookingForPayment) {
      try {
        let updatedBooking;
        if (editingPayment) {
          updatedBooking = await api.updatePayment(
          ...
          updatedBooking = await api.addPayment(
            selectedBookingForPayment.id,
            payment
          );
          dispatch({
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 align-top">
                          ...
                          <div className="text-sm text-gray-500">
                            Base: {Number(booking.basePrice).toLocaleString()}{" "}
                            MAD
                          </div>
