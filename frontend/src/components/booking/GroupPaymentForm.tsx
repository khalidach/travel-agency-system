...
export default function GroupPaymentForm({
    ...
    const getInitialFormData = useCallback((): Omit<Payment, "_id" | "id"> => {
        if (payment) {
            return {
                amount: payment.groupAmount || payment.amount,
    ...
    // Calculate maximum allowed amount (Remaining + Current Group Amount if editing)
    const maxAmount = payment
        ? Number(groupRemainingBalance) + Number(payment.groupAmount || payment.amount)
        : Number(groupRemainingBalance);
                ...
                <div>
                    ...
                    <div className="flex justify-between mt-1">
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                            Group Remaining: {groupRemainingBalance.toLocaleString()} MAD
                        </p>
