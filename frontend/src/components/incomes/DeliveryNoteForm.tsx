...
export default function DeliveryNoteForm({
  ...
  const [items, setItems] = useState<FactureItemForm[]>(() => {
    if (initialData?.items && initialData.items.length > 0) {
      try {
        // Determine what type of items we have since they can now be FactureItem or ExpenseItem
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="md:col-span-2">
          ...
          <input
            type="text"
            disabled
            value={initialData?.deliveryNoteNumber || (initialData ? "-" : t("autoGenerated", { defaultValue: "Auto-generated" }))}
